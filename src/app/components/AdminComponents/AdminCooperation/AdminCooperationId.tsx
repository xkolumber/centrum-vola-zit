...
const AdminCooperationId = ({ id }: Props) => {
  const queryClient = useQueryClient();
  const { data, error, isLoading } = useQuery<CooperationInterface>({
    queryKey: ["admin_spolupracujeme", id],
    queryFn: () => fetchCooperationId(id),
  ...
  const handleSaveData = async (e: any) => {
    ...
    try {
      const response = await AdminActualizeCooperation(actualizeData);
      if (response === 200) {
...
export default AdminCooperationId;
